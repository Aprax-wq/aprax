...
class Message {
  ...
  async useCommands () {
    ...
    if (message.channel.type == "dm" || !Config.Guild.Prefixes.some(x => this.Message.content.startsWith(x.toLowerCase()))) return;
    const prefix = Config.Guild.Prefixes.find(x => this.Message.content.toLowerCase().startsWith(x));
